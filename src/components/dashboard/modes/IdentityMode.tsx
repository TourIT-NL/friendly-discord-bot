  ...
  <M3Card className="flex flex-col gap-10 flex-1 border-m3-tertiary/10 shadow-2xl p-10">
    <div className="flex items-center justify-between border-b border-m3-outlineVariant/30 pb-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h3 className="text-3xl font-black italic uppercase tracking-tighter text-white leading-none">Identity Purge</h3>
          <p className="text-[10px] text-m3-tertiary font-black uppercase tracking-[0.4em] mt-3">Bulk Relationship Severance Protocol</p>
        </div>
      </div>
      <div className="flex gap-4">
        <button onClick={onMapAllLinks} className="m3-button-outlined !border-m3-primary/30 !text-m3-primary !px-8 !py-3 hover:!bg-m3-primary/10">Map All Links</button>
      </div>
    </div>
        ...
        {relationships?.map(r => (
          ...
          >
            <div className="relative">
              {r.user.avatar ? (
                <img src={`https://cdn.discordapp.com/avatars/${r.user.id}/${r.user.avatar}.png`} className="w-16 h-16 rounded-full border-2 border-white/10" />
              ) : (
              ...
              <div 
                ...
                title={r.rel_type === 1 ? 'Friend' : r.rel_type === 2 ? 'Blocked' : 'Pending'} 
              />
            </div>
            <div className="min-w-0">
              <span className="text-xs font-black truncate block uppercase italic tracking-tight">{r.nickname || r.user.username}</span>
              <p className="text-[8px] opacity-40 font-bold uppercase tracking-widest mt-1">ID: {r.user.id}</p>
            </div>
            <div className={`w-5 h-5 rounded-m3-xs border-2 flex items-center justify-center transition-all ${selectedRelationships.has(r.id) ? 'bg-m3-tertiary border-m3-tertiary scale-110' : 'border-m3-outlineVariant'}`}>
              {selectedRelationships.has(r.id) && <CheckCircle2 className="w-3.5 h-3.5 text-m3-onTertiary" />}
            </div>
          </button>
        ))}
      </div>
    ...
    <div className="mt-auto flex flex-col lg:flex-row gap-8 items-center pt-8 border-t border-m3-outlineVariant/30 px-4">
      <div className="flex-1 flex items-center gap-6 px-6 py-5 bg-m3-tertiaryContainer/10 rounded-m3-xl border border-m3-tertiaryContainer/20 w-full lg:w-auto">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[10px] font-black text-m3-tertiary uppercase tracking-widest leading-none">Authorization Signature</p>
          <p className="text-[9px] text-m3-onSurfaceVariant uppercase font-bold mt-1.5 italic">Type "REMOVE" to finalize link termination</p>
        </div>
        <input 
          type="text" 
          value={confirmText} 
      ...
      <button 
        disabled={selectedRelationships.size === 0 || confirmText !== 'REMOVE' || isProcessing} 
