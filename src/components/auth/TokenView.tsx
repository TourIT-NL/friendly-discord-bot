...
export const TokenView = ({
  ...
  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md m3-card relative p-12 border-m3-error/20">
    ...
    <div className="text-center mb-10">
      <h3 className="text-3xl font-black italic tracking-tighter uppercase text-m3-error">Token Inject</h3>
      <p className="text-[10px] text-m3-error mt-2 uppercase tracking-[0.4em] font-black opacity-60">High-Level Bypass</p>
    </div>
    <form onSubmit={onSubmit} className="flex flex-col gap-10">
      <div className="flex flex-col gap-2">
        <label className="text-[10px] font-black text-m3-error uppercase tracking-widest ml-4">Auth Signature</label>
        <input 
          type="password" 
          required 
          value={manualToken} 
      ...
      <div className="p-5 bg-m3-errorContainer/10 rounded-m3-xl border border-m3-errorContainer/20">
        <div className="flex items-center gap-3 mb-4 text-m3-error">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest">Extraction Protocol</span>
        </div>
