...
import { Monitor, Smartphone, Globe, Shield, Key, ChevronRight, ShieldCheck, Settings } from 'lucide-react';
...
export const LoginSelection = ({
  ...
  >
    <div className="space-y-6">
      <M3Card className="flex flex-col gap-6 p-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-m3-onSurfaceVariant">Zero-config link via desktop app</p>
          </div>
        </div>
        <div className="flex items-center justify-between p-4 bg-m3-surfaceVariant/50 rounded-m3-lg border border-m3-outlineVariant shadow-inner">
          <div className="flex items-center gap-3">
            <div className={`w-2 h-2 rounded-full ${discordStatus?.is_running ? 'bg-green-500 shadow-[0_0_8px_green]' : 'bg-m3-outline'}`} />
            <span className="text-xs font-bold uppercase tracking-wider">Discord Process</span>
          </div>
      ...
      <M3Card onClick={onLoginQR} className="flex flex-col items-center gap-4 text-center group py-8">
        ...
        <div>
          ...
          <p className="text-xs text-m3-onSurfaceVariant mt-2 uppercase tracking-widest font-bold">Mobile bridge</p>
        </div>
      </M3Card>
    </div>
    ...
    <div className="space-y-6">
      <div className="m3-card-elevated flex flex-col gap-6 !bg-m3-primaryContainer !text-m3-onPrimaryContainer border-none relative overflow-hidden h-full shadow-2xl p-6">
        ...
        <p className="text-sm leading-relaxed opacity-80 relative z-10 flex-1">
          Standard linkage using the official Discord authorization protocol. Requires Application ID and Secret (fixed port 58123).
        </p>
        <div className="flex gap-3 relative z-10">
          <button 
            onClick={onSwitchToSetup} 
            className="p-3 rounded-m3-full bg-m3-onPrimaryContainer/10 hover:bg-m3-onPrimaryContainer/20 transition-colors focus:outline-none"
          >
          ...
          <button 
            ...
            disabled={isLoading} 
            className="m3-button-primary flex-1 !bg-m3-onPrimaryContainer !text-m3-primaryContainer shadow-xl"
          >
      ...
      <M3Card onClick={onSwitchToToken} className="flex items-center justify-between group py-6 px-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-m3-onSurfaceVariant mt-1 uppercase tracking-widest font-bold">Manual Injection</p>
          </div>
        </div>
