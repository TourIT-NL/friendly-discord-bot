...
import { Guild, Channel, Relationship, Progress, OperationStatus, DiscordUser } from '../types/discord';

export const useDiscordOperations = (handleApiError: (err: any, fallback: string) => void) => {
  const { 
    user, guilds, setGuilds, setError, setLoading, setAuthenticated 
  } = useAuthStore();
  ...
  const [channelsByGuild, setChannelsByGuild] = useState<Map<string, Channel[]>>(new Map());
  ...
  const [operationStatus, setOperationStatus] = useState<OperationStatus>({ 
    is_running: false, is_paused: false, should_abort: false 
  });
  ...
  const handleToggleGuildSelection = async (guild: Guild | null) => {
    ...
    if (!selectedGuilds.has(effectiveId)) {
      setLoading(true);
      try {
        const fetchedChannels: Channel[] = await invoke('fetch_channels', { guildId: guild?.id || null });
        ...
        handleApiError(err, `Failed to load buffers for ${guild?.name || 'Direct Messages'}.`);
  ...
  const handleBuryAuditLog = async () => {
    if (selectedGuilds.size === 0 || selectedChannels.size === 0) {
      setError("Please select at least one guild and one channel for audit log burial.");
      return;
    }
  ...
  const handleWebhookGhosting = async () => {
    if (selectedGuilds.size === 0) { setError("Please select a guild for webhook ghosting."); return; }
    const guildId = Array.from(selectedGuilds)[0];
    if (guildId === 'dms') {
  ...
  return {
    mode, setAppMode,
    selectedGuilds, setSelectedGuilds,
    channelsByGuild, setChannelsByGuild,
    relationships, setRelationships,
    previews, setPreviews,
    selectedChannels, setSelectedChannels,
    selectedGuildsToLeave, setSelectedGuildsToLeave,
    selectedRelationships, setSelectedRelationships,
    isProcessing, setIsProcessing,
    progress, setProgress,
    confirmText, setConfirmText,
    timeRange, setTimeRange,
    searchQuery, setSearchQuery,
    purgeReactions, setPurgeReactions,
    onlyAttachments, setOnlyAttachments,
