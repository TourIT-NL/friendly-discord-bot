...
use tauri::AppHandle;
use tauri::Manager; // Added for app_handle.path()
use std::path::PathBuf; // Added for PathBuf

#[tauri::command]
pub async fn clear_all_app_data(app_handle: AppHandle) -> Result<(), AppError> {
    Logger::info(&app_handle, "[Cleanup] Starting full application reset...", None);
            ...
            Ok(mut entries) => {
                while let Some(entry) = entries.next_entry().await.transpose() {
                    if let Ok(entry) = entry {
                        let path = entry.path();
                        if path.is_file() && path.file_name().and_then(|name| name.to_str()).map_or(false, |name| name.starts_with("app.log")) {
                            // Try to delete. If it fails (e.g. locked), just log and continue.
                            if let Err(e) = tokio::fs::remove_file(&path).await {
                                Logger::debug(
                                    &app_handle,
                                    &format!("[Cleanup] Could not delete log file {} (likely in use): {}", path.display(), e),
                                    None,
                                );
                            } else {
                                Logger::debug(
    ...
    Logger::info(&app_handle, "[Cleanup] Application reset complete. Session terminated.", None);
    Ok(())
}
