...
async fn download_file(
    ...
    let response = api_handle
        ...
        .send()
        .await
        .map_err(|e| AppError::new(&format!("Download failed: {}", e), "download_error"))?;

    let bytes = response.bytes().await.map_err(|e| AppError::new(&format!("Failed to get bytes: {}", e), "io_error"))?;
    
    let mut file = File::create(save_path).map_err(|e| AppError::new(&format!("Failed to create file: {}", e), "io_error"))?;
    file.write_all(&bytes).map_err(|e| AppError::new(&format!("Failed to write file: {}", e), "io_error"))?;
...
pub async fn start_attachment_harvest(
    ...
    if !output_dir.exists() {
        fs::create_dir_all(&output_dir).map_err(|e| AppError::new(&format!("Failed to create output dir: {}", e), "io_error"))?;
    }
    ...
    for (i, channel_id) in options.channel_ids.iter().enumerate() {
        ...
        loop {
            ...
            if let Some(id) = &last_id { url.push_str(&format!("&before={}", id)); }

            let res = api_handle.send_request(reqwest::Method::GET, &url, None, &token, is_bearer).await?;
            let messages: Vec<serde_json::Value> = serde_json::from_value(res).map_err(AppError::from)?;

            if messages.is_empty() { break; }
            last_id = messages.last().and_then(|m| m["id"].as_str().map(|s| s.to_string()));
            ...
            for msg in messages {
                ...
                if let Some(attachments) = msg["attachments"].as_array() {
                    for att in attachments {
                        ...
                        if let Err(e) = download_file(&api_handle, file_url, &save_path).await {
                            Logger::error(&app_handle, &format!("Failed to download attachment: {}", e.user_message), None);
                        } else {
                            total_processed += 1;
                        }
                    }
...
pub async fn start_chat_html_export(
    ...
    for (i, channel_id) in options.channel_ids.iter().enumerate() {
                ...
                if options.include_attachments {
                    if let Some(atts) = msg["attachments"].as_array() {
                        for att in atts {
                            ...
                            html_content.push_str(&format!("<div class='attachment'>Attachment: {}</div>", att_name));
                        }
                    }
                }
        ...
        let mut file = File::create(file_path).map_err(|e| AppError::new(&format!("Failed to create HTML file: {}", e), "io_error"))?;
        file.write_all(html_content.as_bytes()).map_err(|e| AppError::new(&format!("Failed to write HTML: {}", e), "io_error"))?;
    }
...
pub async fn start_guild_user_archive(
    ...
    // 1. Fetch all channels in guild
    let channels_url = format!("https://discord.com/api/v9/guilds/{}/channels", guild_id);
    let channels_res = api_handle.send_request(reqwest::Method::GET, &channels_url, None, &token, is_bearer).await?;
    let channels: Vec<serde_json::Value> = serde_json::from_value(channels_res).map_err(AppError::from)?;

    let output_file = File::create(&output_path).map_err(|e| AppError::new(&format!("Failed to create zip: {}", e), "io_error"))?;
    ...
    for (i, channel) in channels.iter().enumerate() {
        ...
        if c_type != 0 && c_type != 2 { continue; } // Only text and voice channels
        ...
        loop {
            ...
            let res_res = api_handle.send_request(reqwest::Method::GET, &url, None, &token, is_bearer).await;
            let res = match res_res {
                Ok(v) => v,
                Err(_) => break, // Forbidden or other error, skip
            };
            let messages: Vec<serde_json::Value> = serde_json::from_value(res).map_err(AppError::from)?;
...
", ts, channel_name, content));
    ...
    zip.start_file("my_messages.txt", zip_options).map_err(|e| AppError::new(&format!("Zip error: {}", e), "io_error"))?;
    zip.write_all(log_accumulator.as_bytes()).map_err(|e| AppError::new(&format!("Zip write error: {}", e), "io_error"))?;
    zip.finish().map_err(|e| AppError::new(&format!("Zip finish error: {}", e), "io_error"))?;
