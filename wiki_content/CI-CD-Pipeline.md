...
4.  **`release`**: Triggered exclusively on a git tag (e.g., `v1.0.3`), this job orchestrates the final release process. It utilizes the `build` job's artifacts and leverages Tauri's `gh-release` action to create a new GitHub Release, attaching all compiled binaries of the **Discord message deletion tool**.
